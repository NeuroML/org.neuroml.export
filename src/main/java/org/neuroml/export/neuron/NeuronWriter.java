                ...
                for(Component conn : projection.getAllChildren())
                {
                    if(conn.getComponentType().isOrExtends(NeuroMLElements.CONNECTION))
                    {
                        ...
                        else
                        {
                            ...
                            main.append(String.format(Locale.US, bIndent+"h(\"%s nc_%s_%d = new NetCon(%s, %s, %s, %s, %s)\")  \n\n", preSecName, synObjName, index, sourceVarToListenFor, synObjName, threshold, delay, weight));
                        }
                        index++;
                    }
                }
...
//            inputName += "_" + popName + "_" + cellNum + "_" + secName.replaceAll("\\[", "").replaceAll("\\]", "").replaceAll("\\.", "_");
    ...
    private void processTimeDependentLiterals(StringBuilder main, Component input, Component inputComp)
        ...
        String synFullName =  "self."+inputName + "_" + synSafeName+"_"+input.getID();
        main.append(String.format(bIndent+"%s = h.%s(%s, sec=h.%s)\n",
                                        synFullName,
                                        synSafeName,
                                        parseFractionAlong(input),
                                        parseInputSecName(input)));
        String helperFunc = bIndent+"def singleNetStimT(tstim):\n"+bIndent+"\tn=h.NetStim()\n"+bIndent+"\tn.number = 1\n"+bIndent+"\tn.start=tstim\n"+bIndent+"\treturn n\n";
        ...
        String stimName = String.format("self.%s_stims_%s", inputName,input.getID());
        //stimName = String.format("%s_stims", inputName);
        main.append(String.format(bIndent+"%s = [singleNetStimT(t) for t in %s]\n", stimName, spkTimes));
        main.append(String.format(bIndent+"self.%s_netCons_%s = [h.NetCon(s, %s, 0, 0, 1) for s in %s]\n", inputName, input.getID(), synFullName, stimName));
    ...
    public String generateModFile(Component comp, ChannelConductanceOption condOption) throws LEMSException
    {
        ...
        if(comp.getComponentType().isOrExtends(NeuroMLElements.BASE_ION_CHANNEL_COMP_TYPE))
        {
            ...
            hasCaDependency = checkCaDependency(comp);
...
//        }
        ...
        String ratesInvocation = ratesFunctionCall(comp);
        blockInitial.append(ratesInvocation + "\n");
        blockInitial.append(ratesInvocation + "? To ensure correct initialisation.\n");
        ...
        {
            blockBreakpoint.insert(0, ratesInvocation + "\n");
        }
        ...
        if(blockDerivative.length() > 0)
        {
            blockDerivative.insert(0, ratesInvocation + "\n");
            writeModBlock(mod, "DERIVATIVE states", blockDerivative.toString());
        }

        writeModBlock(mod, "PROCEDURE " + ratesInvocation, ratesMethod.toString());
    ...
    private boolean checkCaDependency(Component comp) {
        boolean hasCaDependency = false;
        ...
        return hasCaDependency;
    }

    private String ratesFunctionCall(Component comp) {
        boolean hasCaDependency = checkCaDependency(comp);
        ComponentType type = comp.getComponentType();
        List<String> args = new ArrayList<String>();
        if (type.isOrExtends(NeuroMLElements.BASE_VOLT_DEP_CURR_SRC_SPIKING_COMP_TYPE) ||
                        type.isOrExtends(NeuroMLElements.BASE_ION_CHANNEL_COMP_TYPE))
            args.add("v");
        if(hasCaDependency)
            args.add("cai");
        return "rates(" + StringUtils.join(args, ',') + ")";
    }
    ...
    private void parseKS(Component comp, StringBuilder blockKinetic, String prefix) throws ContentError {
        ...
        blockKinetic.insert(0, ratesFunctionCall(comp) + "\n\n");
